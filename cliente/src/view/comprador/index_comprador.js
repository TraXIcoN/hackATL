...
export default function DashboardComprador() {
  ...
  function Ofertas() {
    return dataOfertas.map((data, index) => {
      const correspondingData1 = dataUtilizadores.find(
        (data1) => data1.iduser === data.utilizadoreIduser
      );
      const correspondingDataI = dataInfraestruturas.find(
        (dataI) => dataI.utilizadoreIduser === data.utilizadoreIduser
      );
      const ofertaestado = !dataContratos.some(
        (dataC) => data.idoferta === dataC.ofertaIdoferta
      );
                ...
                <Link to={`/compra/${data.idoferta}`}>
                  <button
                    ...
                    onClick={() =>
                      SendSave(
                        data.duracaovenda,
                        correspondingDataI.idinfraestrutura,
                        data.idoferta,
                        correspondingData1.iduser,
                        currentUser
                      )
                    }
                  >
            ...
            <li className="swiper-slide card card-slide col-3 mx-2 mb-0">
              <div className="card mb-0 rounded-3">
                <div className="card-body">
                  ...
                  <h4 className="my-0 fw-normal mt-3">
                    {correspondingData1.nomeuser}
                  </h4>
                  <ul className="list-unstyled my-3 p-0">
                    ...
                    <li>
                      <p>
                        Produção: {(data.potenciaproduto * 0.8).toFixed(2)} kW
                      </p>
                    </li>
                    <li>
    ...
    return dataUtilizadores.map((dataU, index) => {
      if (dataU.iduser == currentUser) {
        return dataContratos.map((data, index) => {
          if (
            data.utilizadorIdC == currentUser &&
                  ...
                  <div className="col-md-12">
                    <div className="flex-wrap d-flex justify-content-between align-items-center">
                      <div>
                        <h1>Comprador</h1>
                        <p>
                          Aqui poderá efetuar compras de excedente energético e
                          ver as suas estatísticas!
                        </p>
                      </div>
                      <div>
                        <img
